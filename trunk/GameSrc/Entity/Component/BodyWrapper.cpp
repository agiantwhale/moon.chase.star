...
BodyWrapper::BodyWrapper(Entity* const entity) : IPhysics(entity), _body(nullptr),
																 ...
																 _endContactCallback(),
																 _simulateCallbackSet( false ),
																 _beginContactCallbackSet( false ),
																 _endContactCallbackSet( false )
{
}
...
void BodyWrapper::Simulate( void )
{
	if( _simulateCallbackSet )
	_simulateCallback();
}
...
void BodyWrapper::BeginContact(b2Contact* contact, const b2Fixture* contactFixture )
{
	if( _beginContactCallbackSet )
	_beginContactCallback(contact, contactFixture);
}

void BodyWrapper::EndContact(b2Contact* contact, const b2Fixture* contactFixture )
{
	if( _endContactCallbackSet )
	_endContactCallback(contact, contactFixture);
}

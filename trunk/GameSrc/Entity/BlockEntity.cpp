...
BlockEntity::BlockEntity() : BaseClass(), m_shouldFall(false), m_blockSprite(), m_bodyController(), m_screenPositioner(*this)
{}

BlockEntity::~BlockEntity()
{
	sb::GraphicsManager::getInstance()->removeDrawable(m_blockSprite,2);
	sb::PhysicsManager::getInstance()->getPhysicsWorld()->DestroyBody(m_bodyController.getBody());
}

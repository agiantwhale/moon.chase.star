...
public:
    ...
    inline const Vec2D& GetPosition( void ) const { return _position; }
	inline const Vec2D& GetScale( void ) const { return _scale; }
	inline float GetRotation( void ) const { return _rotation; }

	inline void  SetPosition(const Vec2D& position) {_position=position;}
	inline void  SetScale(const Vec2D& scale) {_scale=scale;}
